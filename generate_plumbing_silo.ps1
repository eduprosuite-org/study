# PowerShell script to programmatically generate 31-page SEO silo structure for plumbing exam prep
$baseDir = Join-Path $PSScriptRoot "exams/plumbing-license-prep"

$silos = @{
    "journeyman" = Join-Path $baseDir "journeyman"
    "master-contractor" = Join-Path $baseDir "master-contractor"
    "tradesman-other" = Join-Path $baseDir "tradesman-other"
    "general" = Join-Path $baseDir "general"
}

# Create base directories
if (-not (Test-Path $baseDir)) { New-Item -ItemType Directory -Force -Path $baseDir }
foreach ($path in $silos.Values) {
...
"@
        questions = @(
            ...
            @{ Category = "USBC Amendments"; Question = "Under Virginia Uniform Statewide Building Code rules, what is the maximum water temperature from a shower head?"; OptionA = "110F"; OptionB = "120F"; OptionC = "130F"; OptionD = "140F"; CorrectOption = "OptionB"; Explanation = "The USBC restricts shower outlet temperatures to a maximum of 120F (49C) to prevent scalding." },
...
# 1. Generate Main Category Hub index.html
$hubHtml = @"
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <section id="categories" style="margin-top: 3rem;">
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 2rem;">
                <article class="glass-card" style="display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    <a href="journeyman/index.html" class="btn-primary" style="text-align: center; text-decoration: none; padding: 0.8rem;">Explore Journeyman Preps</a>
                </article>
                ...
                <article class="glass-card" style="display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    <a href="master-contractor/index.html" class="btn-primary" style="text-align: center; text-decoration: none; padding: 0.8rem;">Explore Advanced Preps</a>
                </article>
                ...
                <article class="glass-card" style="display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    <a href="tradesman-other/index.html" class="btn-primary" style="text-align: center; text-decoration: none; padding: 0.8rem;">Explore Residential Preps</a>
                </article>
                ...
                <article class="glass-card" style="display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    <a href="general/index.html" class="btn-primary" style="text-align: center; text-decoration: none; padding: 0.8rem;">Explore Free Preps</a>
                </article>
            </div>
        </section>
                </div>
...
[System.IO.File]::WriteAllText((Join-Path $baseDir "index.html"), $hubHtml)

# 2. Generate Silo Sub-Hub index.html pages
function Get-SiloHubHtml($siloKey, $title, $desc, $items) {
    $linksHtml = ""
    foreach ($item in $items) {
        ...
        $priceTag = ""
        $paypalBtn = ""
        if ($itemPrice -ne "Free") {
            $priceTag = "<span style=`"background: rgba(16, 185, 129, 0.1); color: #34d399; padding: 0.2rem 0.6rem; border-radius: 4px; font-weight: 700; font-size: 0.85rem;`">$itemPrice</span>"
            $paypalBtn = "<a href=`"https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=youlearneasily@gmail.com&item_name=$itemName&amount=49.99&currency_code=USD`" class=`"btn-primary`" style=`"background: #0070ba; color: white; border: none; padding: 0.7rem; text-align: center; text-decoration: none; font-size: 0.9rem; flex-grow: 1;`">Buy with PayPal</a>"
        } else {
            $priceTag = "<span style=`"background: rgba(6, 182, 212, 0.1); color: #22d3ee; padding: 0.2rem 0.6rem; border-radius: 4px; font-weight: 700; font-size: 0.85rem;`">Free</span>"
        }
        ...
        <article class="glass-card" style="display: flex; flex-direction: column; justify-content: space-between;">
            <div>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem;">
                    <h3 style="font-size: 1.3rem; font-weight: 700; color: white; margin: 0;">$itemName</h3>
                    $priceTag
                </div>
                <p style="color: var(--text-secondary); font-size: 0.9rem; margin-bottom: 1.5rem;">$itemDesc</p>
            </div>
            <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                <a href="$itemId/index.html" class="btn-primary" style="flex-grow: 1; text-align: center; text-decoration: none; padding: 0.7rem; font-size: 0.9rem;">Details</a>
                <a href="$itemId/practice.html" class="btn-primary" style="background: rgba(255,255,255,0.05); border: 1px solid var(--bg-card-border); color: white; box-shadow: none; padding: 0.7rem; text-decoration: none; text-align: center; font-size: 0.9rem; flex-grow: 1;">Practice</a>
                $paypalBtn
            </div>
        </article>
"@
...
<body>
    ...
    <main>
        ...
        <nav aria-label="Breadcrumb" class="breadcrumbs" style="margin-bottom: 2rem; font-size: 0.9rem; color: var(--text-secondary);">
            ...
            <span style="color: var(--text-primary);">$($siloKey.ToUpper()) Silo</span>
        </nav>

        <section class="hero" id="home">
            <h1>$($siloKey.ToUpper()) Exam Packages</h1>
            <p>$desc</p>
        </section>
...
foreach ($siloName in $silos.Keys) {
    $filteredItems = @()
    foreach ($p in $products) {
        if ($p.silo -eq $siloName) {
            $filteredItems += $p
        }
    }
    $title = "$($siloName.Replace('-', ' ').ToUpper()) Plumbing License Exams"
    $desc = "Access specialized mock tests and guides inside our $siloName category silo. Start preparing today."
    $html = Get-SiloHubHtml $siloName $title $desc $filteredItems
    [System.IO.File]::WriteAllText((Join-Path $silos[$siloName] "index.html"), $html)
}

# 3. Product Details and Practice pages generator
function Get-ProductIndexHtml($p) {
    $faqHtml = ""
    $faqSchemaItems = @()
    foreach ($faq in $p.faqs) {
        ...
        <div style="border-top: 1px solid rgba(255,255,255,0.04); padding-top: 1.5rem;">
            <h3 style="font-size: 1.1rem; color: var(--primary); margin-bottom: 0.5rem; font-weight: 600;">$q</h3>
            <p style="color: var(--text-secondary); font-size: 0.95rem; line-height: 1.6;">$a</p>
        </div>
"@
    ...
    $priceTag = $p.price
    $priceVal = $priceTag.Trim("$")
    $paypalBtn = ""
    
    if ($priceTag -ne "Free") {
        $paypalBtn = @"
        <div style="border-top: 1px solid var(--bg-card-border); padding-top: 1.5rem; margin-bottom: 1.5rem; text-align: center;">
            <p style="color: var(--text-primary); font-size: 0.95rem; font-weight: 600; margin-bottom: 1rem;">Unlock Full Premium Bank via PayPal</p>
            <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=youlearneasily@gmail.com&item_name=$($p.name)&amount=$priceVal&currency_code=USD" class="btn-primary" style="display: block; width: 100%; text-align: center; text-decoration: none; padding: 1rem; font-size: 1.1rem; font-weight: 700; background: #0070ba; color: white; border: none; border-radius: 8px;">Secure Checkout with PayPal</a>
        </div>
"@
    } else {
        $paypalBtn = @"
...
<body>
    ...
    <main>
        ...
        <nav aria-label="Breadcrumb" class="breadcrumbs" style="margin-bottom: 2rem; font-size: 0.9rem; color: var(--text-secondary);">
            ...
            <a href="../index.html" style="color: var(--primary); text-decoration: none;">$($p.silo.ToUpper())</a> &gt; 
            ...
            <div>
                <section class="glass-card">
                    ...
                    <div style="display: flex; gap: 1rem; align-items: center; border-top: 1px solid var(--bg-card-border); padding-top: 1.5rem;">
                        <a href="practice.html" class="btn-primary" style="text-decoration: none; padding: 0.9rem 2rem;">Start Practice Test Now</a>
                    </div>
                </section>
            ...
            <div class="stats-panel">
                <div class="glass-card" style="position: sticky; top: 120px;">
                    <div style="text-align: center; margin-bottom: 1.5rem;">
                        ...
                        <div style="font-size: 2.5rem; font-weight: 800; color: white; margin: 1rem 0 0.5rem;">$priceTag</div>
...
function Get-ProductPracticeHtml($p) {
    $content = @"
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <div class="dashboard-grid">
            <div>
                ...
                <div class="glass-card" id="quiz-board">
                    ...
                    <div class="actions-row" id="next-btn-container" style="display: none;">
                        <button class="btn-primary" id="next-btn" onclick="nextQuestion()">Next Question </button>
                    </div>
                </div>
            </div>
            ...
            <div class="stats-panel">
                <div class="glass-card" style="padding: 1.5rem;">
                    ...
                    <button class="btn-primary" id="reset-stats-btn" onclick="resetStats()" style="width: 100%; margin-top: 1.5rem; background: rgba(255,255,255,0.05); border: 1px solid var(--bg-card-border); box-shadow: none; padding: 0.8rem;">Reset Progress</button>
                </div>
...
# Write each product's directories, index.html, practice.html, and questions.csv
foreach ($p in $products) {
    $folderPath = Join-Path $silos[$p.silo] $p.id
    if (-not (Test-Path $folderPath)) { New-Item -ItemType Directory -Force -Path $folderPath }
    
    # Details index.html
    $pIndex = Get-ProductIndexHtml $p
    [System.IO.File]::WriteAllText((Join-Path $folderPath "index.html"), $pIndex)
    
    # Practice practice.html
    $pPractice = Get-ProductPracticeHtml $p
    [System.IO.File]::WriteAllText((Join-Path $folderPath "practice.html"), $pPractice)
    
    # Questions questions.csv
    $csvContent = "Category,Question,OptionA,OptionB,OptionC,OptionD,CorrectOption,Explanation`n"
    foreach ($q in $p.questions) {
        $cat = $q.Category -replace '"', '""'
        $quest = $q.Question -replace '"', '""'
        $optA = $q.OptionA -replace '"', '""'
        $optB = $q.OptionB -replace '"', '""'
        $optC = $q.OptionC -replace '"', '""'
        $optD = $q.OptionD -replace '"', '""'
        $correct = $q.CorrectOption
        $exp = $q.Explanation -replace '"', '""'
        
        $csvContent += "`"$cat`",`"$quest`",`"$optA`",`"$optB`",`"$optC`",`"$optD`",`"$correct`",`"$exp`"`n"
    }
    
    [System.IO.File]::WriteAllText((Join-Path $folderPath "questions.csv"), $csvContent)
}
